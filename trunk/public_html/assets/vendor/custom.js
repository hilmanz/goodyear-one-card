...
jQuery(document).ready(function($){
    ...
    $('#btndata').click(function(e){
		...
		if(alamat.length == 0){
	        var error = true;
	        $('#alamat_error').addClass("error");
	    }else{
	        $('#alamat_error').removeClass("error");
	    }
		if(provinsi.length == 0){
	        var error = true;
	        $('#provinsi_error').addClass("error");
	    }else{
	        $('#provinsi_error').removeClass("error");
	    }
		if(kodepos.length == 0){
	        var error = true;
	        $('#kodepos_error').addClass("error");
	    }else{
	        $('#kodepos_error').removeClass("error");
	    }
        if(error == false){
	    	owl.trigger('owl.next');
        }
