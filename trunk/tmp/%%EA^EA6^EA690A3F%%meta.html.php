<?php /* Smarty version 2.6.13, created on 2016-05-04 07:57:39
         compiled from application/web//meta.html */ ?>
<meta charset="UTF-8" />
        <meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1"> 
        <title>GOODYEAR - #ONEGOODCARD</title>
        <meta name="viewport" content="width=device-width, initial-scale=1.0"> 
        <meta name="description" content="GOODYEAR - #ONEGOODCARD" />
        <meta name="keywords" content="GOODYEAR,ONEGOODCARD" />
        <meta name="author" content="ACITJAZZ" />
      	<link rel="shortcut icon" href="<?php echo $this->_tpl_vars['basedomain']; ?>
assets/images/favicon.png"> 
        <link rel="stylesheet" type="text/css" href="<?php echo $this->_tpl_vars['basedomain']; ?>
assets/vendor/bootstrap/css/bootstrap.css" />
        <link rel="stylesheet" type="text/css" href="<?php echo $this->_tpl_vars['basedomain']; ?>
assets/css/animate.css" />
        <link rel="stylesheet" type="text/css" href="<?php echo $this->_tpl_vars['basedomain']; ?>
assets/vendor/eufont/eufont.css" />
		<link rel="stylesheet" type="text/css" href="<?php echo $this->_tpl_vars['basedomain']; ?>
assets/vendor/magnific-popup/magnific-popup.css" />
        <link rel="stylesheet" type="text/css" href="<?php echo $this->_tpl_vars['basedomain']; ?>
assets/vendor/owlcarousel/owl.carousel.css" />
        <link rel="stylesheet" type="text/css" href="<?php echo $this->_tpl_vars['basedomain']; ?>
assets/vendor/owlcarousel/owl.theme.css" />
        <link rel="stylesheet" type="text/css" href="<?php echo $this->_tpl_vars['basedomain']; ?>
assets/vendor/owlcarousel/owl.transitions.css" />
        <link rel="stylesheet" type="text/css" href="<?php echo $this->_tpl_vars['basedomain']; ?>
assets/fonts/bebas.css" />
        <link rel="stylesheet" type="text/css" href="<?php echo $this->_tpl_vars['basedomain']; ?>
assets/css/onecard.css" />
        <link rel="stylesheet" type="text/css" href="<?php echo $this->_tpl_vars['basedomain']; ?>
assets/css/responsive.css" />
		<script src="<?php echo $this->_tpl_vars['basedomain']; ?>
assets/vendor/jquery.js"></script>
		<script src="<?php echo $this->_tpl_vars['basedomain']; ?>
assets/vendor/jquery-ui.js"></script>
		<script src="<?php echo $this->_tpl_vars['basedomain']; ?>
assets/vendor/bootstrap/js/bootstrap.js"></script>
		<script type="<?php echo $this->_tpl_vars['basedomain']; ?>
text/javascript" src="assets/vendor/modernizr.js"></script>
<title>GOODYEAR</title>

<script>
		var basedomain = "<?php echo $this->_tpl_vars['basedomain']; ?>
" ;
		var basedomainpath = "<?php echo $this->_tpl_vars['basedomainpath']; ?>
" ;
		var pages = "<?php echo $this->_tpl_vars['pages']; ?>
" ;
		var radiusmeter = "<?php echo $this->_tpl_vars['radiusmeter']; ?>
" ;
		var countregister ="<?php echo $this->_tpl_vars['countregister']; ?>
";
	</script> 
