...
class App extends Application{
	...
	function main(){
		...
		if($CONFIG['MAINTENANCE']==true){
			...
			$this->assign('localasset', $CONFIG['LOCAL_PUBLIC_ASSET']);
			...
			// pr($this->user);die;
			$this->assign('radiusmeter',$CONFIG['RADIUS_METER']);
			$this->assign('role',$this->user->role);
			...
			else if($this->user->role==2)
			{
				...
				$this->assign('header',$this->View->toString(TEMPLATE_DOMAIN_WEB . "/widgets/headerMember.html"));
			}
			else
			{
