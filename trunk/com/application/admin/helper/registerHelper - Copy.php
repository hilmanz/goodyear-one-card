...
class registerHelper {
	...
	function getalluserhirarkies(){
		$userhirarkies = false;
		//get active 121,is,agency
		$sql = "	SELECT * FROM my_pages WHERE type IN (4,6,100)";		
		...
		foreach($branddata as $val){
		 	$hirarkiesid[$val['ownerid']] = $val['ownerid'];
		}
		...
		foreach($hirarkiesid  as  $val){
			 	$hirarkiesid[$val]=$areaid;
		}
		foreach($hirarkiesid  as  $brandid => $branddata){
			foreach($branddata as $areaid => $cityid){
			//get pl of this area and brand
			$sql = "	SELECT * FROM my_pages WHERE type=2 AND ( brandid IN ({$brandid}) OR brandsubid IN ({$brandid}) ) AND areaid IN ({$cityid}) ";		
			// pr($sql);
			$pldata = $this->apps->fetch($sql,1);
			if($pldata){
				// pr($pldata);
					foreach($pldata as $key => $val){
					$hirarkiesid[$brandid][$areaid][$key]= $val['ownerid'];
					}
				}
			}
		}
